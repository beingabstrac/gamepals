...
- **The table**: a 9-foot table, 2540 by 1270 mm, held upright, balls 57 mm across, six pockets with jaws. Balls slow with rolling friction, lose a little on each other and more on the cushions. No spin in this version, which is the first thing to add if the game earns it.
...
- **Two players**, on one phone or against a bot. The table faces the person against a bot; with two people it faces nobody in particular, like a real table, and the names say whose turn it is.

## Tests
The rack is fifteen balls with the 8 in the middle and a solid and a stripe in the back corners; the same shot from the same table always ends the same way; a straight shot into a pocket pockets the ball; a ball does not pass through another; energy only ever goes down; a scratch gives ball in hand; the wrong ball first is a foul; no cushion after contact is a foul; the first legal pocket after the break assigns groups; your own ball in keeps the turn; the 8 early, in the wrong pocket or with a scratch loses; the 8 in the called pocket after your group wins; `accepts` and `apply` agree on every move in `legalMoves` and on shots made up at random; replay reproduces a game; Expert beats Easy in most games. Invariant, after every shot of random games: fifteen object balls and one cue ball are all accounted for, on the table or pocketed; no two balls on the table overlap; every ball on the table is inside the cushions.
