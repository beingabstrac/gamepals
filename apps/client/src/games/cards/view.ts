...
export function makeCard(scene: Scene, card: number, cw: number, ch: number): CardView {
  ...
  //
  // It is kept as narrow as those glyph sizes allow, because a hand fans the other way: a card in
  // a hand shows the strip from its left edge to the next card, so a wide index clips there
  // instead. At this width it reaches 0.41 of the card, so a hand has to pass about twenty cards
  // before the suit goes. Only Old Maid deals that many, and Old Maid is played on rank alone.
  const rankText = sharpText(scene, -cw / 2 + 14 * scale, -ch / 2 + 18 * scale, RANKS[rank - 1]!, (rank === 10 ? 18 : 26) * scale, color).setFontStyle('bold');
  const suitText = sharpText(scene, -cw / 2 + 30 * scale, -ch / 2 + 18 * scale, suit, 16 * scale, color);
