...
- **Green CI is not a picture.** Tests say a game runs and finishes; they cannot say it looks right. Card-game seat names shipped twice running off the table and sitting on the piles. Download the `gallery-iphone` artifact from the run and look at the new game before calling it done, and turn whatever you find into a check the scene can answer (`labelCheck()`, `handCheck()`, `tileCheck()`).
- **A test that plays whole games says how long it needs** (`{ timeout: 120_000 }`). CI runners are slower and busier than a laptop, and two of these have tripped the 5s default at 5030ms and 5103ms, failing a docs-only push. If a new test simulates games, give it a timeout.
...
## Status
M1 in progress (2026-09-13). Live at https://beingabstrac.github.io/gamepals/ (deployed only from a green CI run on main). 41 games: Tic-Tac-Toe, Four in a Row, Ludo, 2048, Sudoku (4 levels, notes, hints), Solitaire (Klondike, draw 1 or 3), FreeCell, Spider (one, two or four suits), Pyramid, TriPeaks, Crazy Eights (2 to 4 players, hidden hands), Go Fish (2 to 4 players), War, Old Maid (2 to 4 players), Hearts (4 players, one hand or to 50 or 100), Spades (4 players in partnerships, bidding and nil), Callbreak (4 players, calls and five rounds), Memory (1 to 4 players, 3 sizes), Sliding Puzzle (3 sizes), Color Sort (3 levels, solver-checked), Echo (solo goals or 2 to 4 player party), Classic Snake (solo, best score), Checkers (English draughts), Chess (our own engine), Backgammon, Sea Battle, Reversi, Dots & Boxes (3 sizes, 2 to 4 players), Mancala (Kalah), Snakes & Ladders (2 to 4 players), Ultimate Tic-Tac-Toe, Yatzy (solo or 2 to 4 players), Shut the Box (9 or 12 tiles), Dominoes (Block or Draw, 2 to 4 players), Air Hockey, Ping Pong (table tennis), Tug of War, Reflex Race, Sumo, Penalty Kicks, Snake Battle. Games can take a level from the table (`levels` in the registry, passed to the rules as `variant`). Wave 3 done; the web app is an installable offline PWA. The card games share one look, one card renderer and one hidden-hand cover (`apps/client/src/games/cards/`), and the trick-taking games share `packages/rules/src/games/tricks.ts`. Work follows the loop-sized milestones in `docs/08-roadmap.md`: each /loop run finishes the first unblocked milestone end to end (green on every screen type, deployed), ticks it, and stops.
