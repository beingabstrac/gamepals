...
## Status
M1 in progress (2026-09-13). Live at https://beingabstrac.github.io/gamepals/ (deployed only from a green CI run on main). 15 games: Tic-Tac-Toe, Four in a Row, Ludo, 2048, Sudoku (4 levels, notes, hints), Solitaire (Klondike, draw 1 or 3), Memory (1 to 4 players, 3 sizes), Sliding Puzzle (3 sizes), Air Hockey, Ping Pong (table tennis), Tug of War, Reflex Race, Sumo, Penalty Kicks, Snake Battle. Games can take a level from the table (`levels` in the registry, passed to the rules as `variant`). Next: the rest of wave 3 (brief first), bot Web Worker, Capacitor iOS/Android + release-tag phone builds (needs App Store Connect API key from the owner).
