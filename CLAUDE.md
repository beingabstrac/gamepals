...
## Status
M1 in progress (2026-09-13). Live at https://beingabstrac.github.io/gamepals/ (deployed only from a green CI run on main). 35 games: Tic-Tac-Toe, Four in a Row, Ludo, 2048, Sudoku (4 levels, notes, hints), Solitaire (Klondike, draw 1 or 3), FreeCell, Spider (one, two or four suits), Pyramid, TriPeaks, Crazy Eights (2 to 4 players, hidden hands), Memory (1 to 4 players, 3 sizes), Sliding Puzzle (3 sizes), Color Sort (3 levels, solver-checked), Echo (solo goals or 2 to 4 player party), Classic Snake (solo, best score), Checkers (English draughts), Chess (our own engine), Backgammon, Sea Battle, Reversi, Dots & Boxes (3 sizes, 2 to 4 players), Mancala (Kalah), Snakes & Ladders (2 to 4 players), Ultimate Tic-Tac-Toe, Yatzy (solo or 2 to 4 players), Shut the Box (9 or 12 tiles), Dominoes (Block or Draw, 2 to 4 players), Air Hockey, Ping Pong (table tennis), Tug of War, Reflex Race, Sumo, Penalty Kicks, Snake Battle. Games can take a level from the table (`levels` in the registry, passed to the rules as `variant`). Wave 3 done; the web app is an installable offline PWA. The card games share one look and one card renderer (`apps/client/src/games/cards/`). Work follows the loop-sized milestones in `docs/08-roadmap.md`: each /loop run finishes the first unblocked milestone end to end (green on every screen type, deployed), ticks it, and stops.
